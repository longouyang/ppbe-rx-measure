...
how many usable example sequences do i have for each rule?

```{r}
responses %>% group_by(worker.id, rule.id) %>% summarise(all.correct = all(correct)) %>% group_by(rule.id) %>% summarise(num.usable.sequences = sum(all.correct))
```
